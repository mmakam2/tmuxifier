...
test('https public URL marks the cookie Secure even without local TLS', () => {
  const c = loadConfig({}, { env: { TMUXIFIER_BASE_EXTERNAL_URL: 'https://tmuxifier.babendums.com' }, cwd: '/app' });
...
test('public URL without a scheme defaults to https', () => {
  const c = loadConfig({}, { env: { TMUXIFIER_BASE_EXTERNAL_URL: 'tmuxifier.babendums.com/' }, cwd: '/app' });
  expect(c.publicUrl).toBe('https://tmuxifier.babendums.com');
  expect(c.secureCookie).toBe(true);
});
