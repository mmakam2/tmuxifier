...
test('lifecycle and guest fetch methods use exact routes and bodies', async () => {
  const calls = [];
  globalThis.fetch = async (url, opts) => {
    calls.push({ url, opts });
    return { ok: true, status: 200, statusText: 'OK', json: async () => [] };
  };
  await pve.linkedGuests();
  await pve.nodeGuests('H1', 'pve/a');
  ...
  expect(calls.map((call) => call.url)).toEqual([
    '/api/proxmox/guests',
    '/api/proxmox/hosts/H1/nodes/pve%2Fa/guests',
