...
tunnel at `https://tmuxifier.example.com` (TLS terminates at Cloudflare; the
...
| `TMUXIFIER_PUBLIC_URL` | e.g. `https://tmuxifier.example.com` — builds the OAuth `redirect_uri` and marks the cookie `Secure` when its scheme is https |
...
- `docs/DEPLOY.md` — Cloudflare-tunnel + Google Cloud Console setup note:
  authorized redirect URI `https://tmuxifier.example.com/api/auth/google/
