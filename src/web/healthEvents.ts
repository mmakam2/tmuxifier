...
import { classifyError, CPU_ICON, MEM_ICON, DISK_ICON } from './statusDot';
...
// Nerd Font glyphs from the bundled face (Bench Instrument world: the chrome
// speaks one face, and state color comes from the row's LED level, not from
// color emoji). These render only in the in-app panel — browser notifications
// carry line.text alone, so no PUA glyph ever reaches an OS toast.
const METRIC_ICON = { cpu: CPU_ICON, mem: MEM_ICON, disk: DISK_ICON } as const;
...
export function formatEvent(e: HealthEvent): EventLine {
  ...
  switch (e.kind) {
    ...
    case 'agent-input': return { icon: '', text: `${name} — claude is waiting for input`, level: 'warn' };
    ...
    default:
      ...
      return { icon: '', text: `${name} — ${String((e as HealthEvent).kind)}`, level: 'warn' };
  }
}
