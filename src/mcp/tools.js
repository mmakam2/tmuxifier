...
// Mirrors src/server/boxActions.js's TOOL_IDS the same way (the setup
// catalog; `agy` is the Antigravity CLI — the description once said
// `antigravity`, which the server rejects as an unknown tool).
export const SETUP_TOOLS = ['upgrade', 'curl', 'git', 'gh', 'node', 'bubblewrap', 'codex', 'claude', 'agy'];
...
export const TOOL_DEFS = [
  ...
  { name: 'send_text', description: 'Type literal text into the box\'s session. The server sanitizes it first: every run of whitespace — newlines included — collapses to one space and control characters are stripped, so a multi-line prompt arrives as a single line. Set submit=true to press Enter afterwards — use it to send a prompt to a Claude session.', inputSchema: { type: 'object', required: ['box_id', 'text'], properties: { box_id: BOX_ID, text: str('Text to type'), submit: bool('Press Enter after the text (default false)') } } },
  ...
  { name: 'start_setup', description: 'Start a server-side setup job on a box: tmux, optional shell frameworks, the tool catalog, AI-auth seeding, the Claude Code statusline/hooks, and an optional post-setup saved script.', inputSchema: { type: 'object', required: ['box_id'], properties: { box_id: BOX_ID, oh_my_tmux: bool('Install oh-my-tmux'), oh_my_zsh: bool('Install oh-my-zsh'), oh_my_bash: bool('Install oh-my-bash'), tools: { type: 'array', items: { type: 'string', enum: SETUP_TOOLS }, description: 'Tool ids from the setup catalog (agy is the Antigravity CLI)' }, seed_ai_auth: bool('Copy the host\'s AI CLI credentials to the box'), claude_statusline: bool('Push the Claude Code statusline (legacy flag; the claude tool implies it)'), script_id: str('Saved script id to run last'), script_name: str('Display label for that script') } } },
